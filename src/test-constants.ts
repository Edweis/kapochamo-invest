import BinanceInfoNEXT from './functions/watcher/Info';

export const TEST_OPTIMIST_NEWS = BinanceInfoNEXT.fromObject({
  title: 'Introducing the Cartesi (CTSI) Token Sale on Binance Launchpad',
  url:
    'https://binance.zendesk.com/hc/en-us/articles/360041795572-Introducing-the-Cartesi-CTSI-Token-Sale-on-Binance-Launchpad',
  content:
    'Fellow Binancians,\n' +
    'Binance is excited to announce the next project on Binance Launchpad - Cartesi (CTSI). The Cartesi token sale will follow a lottery format, with the recording of user BNB balances starting at 2020/04/14 00:00 AM (UTC). \n' +
    'Binance will record user BNB balances for 7 days from 2020/04/14 00:00 AM (UTC) to 2020/04/21 00:00 AM (UTC). The final BNB holding amount for each user will be determined as the average of the 7 days, but using the Daily Average BNB Balance calculation previously announced here.\n' +
    'CTSI Token Sale Details: \n' +
    '\n' +
    'Token Name: Cartesi (CTSI)\n' +
    'Launchpad Hard Cap: 1,500,000 USD\n' +
    'Total Token Supply: 1,000,000,000 CTSI\n' +
    'Total Tokens Allocated to Binance Launchpad: 100,000,000 CTSI (10% of Total Token Supply)\n' +
    'Public Sale Token Price: 1 CTSI = 0.015 USD (price in BNB will be determined prior to the lottery draw date)\n' +
    'Token Sale Format: Lottery\n' +
    'Max Number of Winning Lottery Tickets: 7,500\n' +
    'Allocation Per Winning Ticket: 200 USD (13,333.33 CTSI)\n' +
    'Supported Sessions: BNB only\n' +
    '\n' +
    'Lottery Timeline:\n' +
    '\n' +
    '2020/04/14 00:00 AM (UTC) to 2020/04/21 00:00 AM (UTC): During this period, user BNB balances will be calculated in hourly snapshots for each day over a 7-day period. Your final average daily BNB balance over these 7 days will determine the number of tickets that you will be able to claim.\n' +
    '2020/04/21 06:00 AM (UTC): Ticket claim will open at this time for all eligible users for a 24 hour period. Users must also sign the Token Purchase Agreement at the same time, prior to completing the ticket claim. Please note that users will only be able to claim tickets once. \n' +
    '2020/04/22 06:00 AM (UTC): Ticket claim closes and the lottery draw begins.\n' +
    '2020/04/22 08:00 AM (UTC): Winning tickets are announced and the respective BNB will be deducted from each winning users’ accounts. Please ensure you have sufficient BNB in your spot exchange account to be deducted within 24 hours if you have a winning ticket. Please note that BNB in open orders, margin accounts, lending products, sub-accounts, futures accounts, or fiat accounts will not be eligible to be deducted.\n' +
    '\n' +
    'Lottery Ticket Allocation\n' +
    'Users will be able to claim up to a maximum of 10 lottery tickets based on their daily average BNB holdings (represented by X below) over the 7-day period leading up to the ticket claim date.\n' +
    'For example, 50 ≤  X < 100 means that your average BNB balance over the entire specified period is kept at 50 BNB or more, but does not exceed 100 BNB. \n' +
    '\n' +
    '\n' +
    '\n' +
    '\n' +
    'Daily Average BNB balance (X)\n' +
    '\n' +
    '\n' +
    'No. of Lottery Tickets\n' +
    '\n' +
    '\n' +
    '\n' +
    '\n' +
    '50 ≤  X < 100\n' +
    '\n' +
    '\n' +
    '1\n' +
    '\n' +
    '\n' +
    '\n' +
    '\n' +
    '100 ≤  X < 150\n' +
    '\n' +
    '\n' +
    '2\n' +
    '\n' +
    '\n' +
    '\n' +
    '\n' +
    '150 ≤  X < 200\n' +
    '\n' +
    '\n' +
    '3\n' +
    '\n' +
    '\n' +
    '\n' +
    '\n' +
    '200 ≤  X < 250\n' +
    '\n' +
    '\n' +
    '4\n' +
    '\n' +
    '\n' +
    '\n' +
    '\n' +
    '250 ≤  X < 300\n' +
    '\n' +
    '\n' +
    '5\n' +
    '\n' +
    '\n' +
    '\n' +
    '\n' +
    '300 ≤  X < 350\n' +
    '\n' +
    '\n' +
    '6\n' +
    '\n' +
    '\n' +
    '\n' +
    '\n' +
    '350 ≤  X < 400\n' +
    '\n' +
    '\n' +
    '7\n' +
    '\n' +
    '\n' +
    '\n' +
    '\n' +
    '400 ≤  X < 450\n' +
    '\n' +
    '\n' +
    '8\n' +
    '\n' +
    '\n' +
    '\n' +
    '\n' +
    '450 ≤  X < 500\n' +
    '\n' +
    '\n' +
    '9\n' +
    '\n' +
    '\n' +
    '\n' +
    '\n' +
    '>=500\n' +
    '\n' +
    '\n' +
    '10\n' +
    '\n' +
    '\n' +
    '\n' +
    '\n' +
    "To learn more about Cartesi (CTSI), read Binance Research's in-depth report here.\n" +
    'For more details on our 13th Launchpad project Cartesi, read our blog post here.\n' +
    'Cartesi Project Links:\n' +
    'Website: https://cartesi.io/\n' +
    'Whitepaper: Cartesi (CTSI)\n' +
    'Telegram: https://t.me/cartesiproject\n' +
    'Twitter: https://twitter.com/cartesiproject \n' +
    'Medium: https://medium.com/cartesi\n' +
    'Discord: https://discordapp.com/invite/Pt2NrnS\n' +
    'Reminder: Users need to complete their account verification and also be from an eligible jurisdiction to participate in this token sale. \n' +
    '* Users from the following countries or regions will not be able to participate in token sales on the Binance Launchpad platform: \n' +
    'Belarus, Democratic Republic of Congo, Cuba, Iraq, Iran, North Korea,  Sudan, Syria, United States of America and its territories (American Samoa, Guam, the Northern Mariana Islands, Puerto Rico, and the U.S. Virgin Islands), Zimbabwe.\n' +
    'Risk warning:\n' +
    'All investments are subject to high market risk. Please make your investments very cautiously. Binance will choose high-quality projects, but will not be responsible for any investment losses.\n' +
    'Thanks for your support!\n' +
    'Binance Team\n' +
    '2020/04/13\n' +
    ' \n' +
    'Trade on the go with Binance’s mobile crypto trading app:\n' +
    'Download for Android\n' +
    'Download for iOS\n' +
    ' \n' +
    'Find us on \n' +
    'Telegram: https://t.me/binanceexchange\n' +
    'Twitter: https://twitter.com/binance\n' +
    'Facebook: https://www.facebook.com/binance \n' +
    'Instagram: https://www.instagram.com/binance',
  time: new Date('2020-04-13 09:53:13+00'),
});
export const TEST_LISTING_NEWS = BinanceInfoNEXT.fromObject({
  title: 'Binance Will List Hive (HIVE)',
  url:
    'https://binance.zendesk.com/hc/en-us/articles/360042363352-Binance-Will-List-Hive-HIVE-',
  content:
    'Fellow Binancians,\n' +
    'Binance will list Hive (HIVE) and open trading for HIVE/BNB, HIVE/BTC and HIVE/USDT trading pairs at 2020/04/27 05:00 AM (UTC). \n' +
    'Due to initial wallet requirements, HIVE deposits and withdrawals will open at 2020/04/29 04:00 AM (UTC). \n' +
    'Please ensure that you undertake sufficient risk assessment when trading newly listed tokens, as they are often subject to high price volatility.\n' +
    'HIVE Listing Fee: 0 BNB.\n' +
    'Details:\n' +
    '\n' +
    'Fees\n' +
    'Rules\n' +
    '\n' +
    'Risk warning: Cryptocurrency trading is subject to high market risk. Please make your trades cautiously. Binance will make best efforts to choose high quality coins, but will not be responsible for your trading losses.\n' +
    'Thanks for your support!\n' +
    'Binance Team\n' +
    '2020/04/27\n' +
    ' \n' +
    'Trade on the go with Binance’s mobile crypto trading app:\n' +
    'Download for Android\n' +
    'Download for iOS\n' +
    ' \n' +
    'Find us on\n' +
    'Telegram: https://t.me/binanceexchange\n' +
    'Twitter: https://twitter.com/binance\n' +
    'Facebook: https://www.facebook.com/binance\n' +
    'Instagram: https://www.instagram.com/binance',
  time: new Date('2020-04-27 03:57:00+00'),
});
